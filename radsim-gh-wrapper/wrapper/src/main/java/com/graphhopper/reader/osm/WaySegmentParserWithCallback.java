/*
 * VENDORED FROM GRAPHHOPPER:
 * graphhopper/core/src/main/java/com/graphhopper/reader/osm/WaySegmentParserWithCallback.java
 *
 * Original license:
 ...
 *
 * MODIFICATIONS:
 * 1. Renamed to WaySegmentParserWithCallback to avoid conflicts
 * 2. Kept in com.graphhopper.reader.osm package to access package-private classes
 * 3. Added SegmentCallback field and setter in Builder
 * 4. Modified Pass2Handler to track segIndex per way and invoke callback before edgeHandler
 */
...
import org.radsim.ghwrapper.importhook.SegmentCallback;
...
/**
 * VENDORED VERSION of WaySegmentParserWithCallback with SegmentCallback support.
 *
 ...
 * {@link OSMNodeData}.
 */
public class WaySegmentParserWithCallback {
    private static final Logger LOGGER = LoggerFactory.getLogger(WaySegmentParserWithCallback.class);
    ...
    private SegmentCallback segmentCallback = (baseWayId, osmNodeIds, segIndex, isBarrierEdge) -> {
    };
    ...
    private WaySegmentParserWithCallback(OSMNodeData nodeData) {
    ...
    private class Pass2Handler implements ReaderElementHandler {
        ...
        private long wayCounter = 0;
        // MODIFICATION: Track current way and segment index for callback
        ...
        public void handleWay(ReaderWay way) {
            ...
            if (!wayFilter.test(way))
                return;
            // MODIFICATION: Reset segment index for each new way
            currentWayId = way.getId();
            currentSegIndex = 0;
        ...
        void handleSegment(List<SegmentNode> segment, ReaderWay way) {
            ...
            // MODIFICATION: Call segmentCallback BEFORE edgeHandler
            // Extract OSM node IDs from segment
            List<Long> osmNodeIds = new ArrayList<>(segment.size());
            for (SegmentNode node : segment) {
                osmNodeIds.add(node.osmNodeId);
            }
            // Check if this is a barrier edge
            boolean isBarrierEdge = Boolean.TRUE.equals(way.getTag("gh:barrier_edge"));
            // Invoke callback with current segment metadata
            segmentCallback.onSegment(currentWayId, osmNodeIds, currentSegIndex, isBarrierEdge);
    ...
    public static class Builder {
        private final WaySegmentParserWithCallback waySegmentParser;
        ...
        public Builder(PointAccess pointAccess, Directory directory) {
            waySegmentParser = new WaySegmentParserWithCallback(new OSMNodeData(pointAccess, directory));
        }
        ...
        /**
         * @param segmentCallback callback function that is called before each edge is created, with OSM node IDs
         */
        public Builder setSegmentCallback(SegmentCallback segmentCallback) {
            waySegmentParser.segmentCallback = segmentCallback;
            return this;
        }
        ...
        public WaySegmentParserWithCallback build() {
